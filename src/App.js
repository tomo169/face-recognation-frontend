...
loadUser = (data) => {
  ...
  onButtonSubmit = () => {
    this.setState({imageUrl: this.state.input});
      fetch('https://nameless-wildwood-10685.herokuapp.com/imageurl', {
        method: 'post',
      ...
      .then(response => {
        if (response) {
          fetch('https://nameless-wildwood-10685.herokuapp.com/image', {
            method: 'put',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({
          ...
          .then(response => response.json())
          .then(count => {
            this.setState(Object.assign(this.state.user, {enteris: count}))
          })
          .catch(console.log)
        }
    ...
    return (
      <div className="App">
        ...
        { this.state.route === 'home'
         ? <div>
         <Logo />
        <Rank name={this.state.user.name} enteris={this.state.user.enteris} />
        <ImageLinkForm 
